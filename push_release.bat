...
set "REPO_URL=https://%GITHUB_USERNAME%:%GH_TOKEN%@github.com/marffinn/equalizer.git"
...
makensis equalizer_installer.nsi
if %errorlevel% neq 0 (
...
move "EqualizerApp_Installer.exe" "release\EqualizerApp_Installer-%APP_VERSION%.exe"
if %errorlevel% neq 0 (
...
git add "release\EqualizerApp_Installer-%APP_VERSION%.exe"
if %errorlevel% neq 0 (
    echo Error: Failed to add installer to git.
    exit /b 1
)
git commit -m "feat(release): add EqualizerApp_Installer-%APP_VERSION%.exe"
if %errorlevel% neq 0 (
